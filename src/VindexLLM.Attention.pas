...
procedure TVdxAttention.Init(const ACompute: TVdxCompute;
  const AHiddenDim: UInt32; const ANumQHeads: UInt32;
  const ANumKVHeads: UInt32; const AHeadDim: UInt32;
  const ANumLayers: UInt32; const AMaxSeqLen: UInt32;
...
begin
  ...
  if not AReader.GetTensorInfo(
    Format('blk.%d.attn_q.weight', [ALayerIndex]), LQInfo) then
  begin
    FErrors.Add(esError, 'AT01',
      'Attention: Q weight not found for layer %d', [ALayerIndex]);
    Exit;
  end;
