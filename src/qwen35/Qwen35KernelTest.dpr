program Qwen35KernelTest;
{$APPTYPE CONSOLE}
uses System.SysUtils, System.Classes, System.IOUtils, Winapi.Windows,
 VindexLLM.Compute,VindexLLM.Vulkan,VindexLLM.Model.Qwen35;
var GPU: TVdxCompute; Layout: VkDescriptorSetLayout; Module: VkShaderModule;
 ...
 S: TResourceStream; F: TFileStream; P: TVdxQwen35Push; Groups: array[0..2] of UInt32;
 Sizes: array[0..4] of UInt32; Buffers: array[0..4] of TVdxGpuBuffer;
...
begin
 try
  ...
  try
   ...
   S := TResourceStream.Create(HInstance,'QWEN35_DENSE',RT_RCDATA);
   try Module := GPU.CreateShaderModule(S.Memory,S.Size); finally S.Free(); end;
   Layout := GPU.CreateStorageDescriptorSetLayout(5);
   ...
   begin
    ...
    Pool := GPU.CreateDescriptorPoolForStorage(1,5);
    ...
    SetLength(Data,Sizes[4]);GPU.DownloadFromBuffer(Buffers[4],@Data[0],Sizes[4]);
    TFile.WriteAllBytes(ChangeFileExt(Name,'.state'),Data);
    GPU.DestroyDescriptorPoolHandle(Pool);
    for I := 0 to 4 do GPU.DestroyGpuBuffer(Buffers[I]);
