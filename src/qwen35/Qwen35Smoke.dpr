...
uses
  ...
  VindexLLM.Model.Qwen35, VindexLLM.Inference, VindexLLM.Model,
...
begin
  try
    if ParamCount = 0 then raise Exception.Create('Usage: Qwen35Smoke model.gguf [prompt] [--tokenize]');
    Prompt := 'Quanto e 2 + 2? Responda apenas o numero.';
    if ParamCount >= 2 then Prompt := ParamStr(2);
    if (ParamCount >= 3) and ((ParamStr(3) = '--tokenize') or (ParamStr(3) = '--tokenize-file')) then
    begin
      if ParamStr(3) = '--tokenize-file' then Prompt := TFile.ReadAllText(Prompt,TEncoding.UTF8);
    ...
    try
      ...
      if not Engine.LoadModel(ParamStr(1),4096) then raise Exception.Create('Load failed: '+Engine.GetErrors.ToString);
